    ...
    if (!empty($nimi) && !empty($kuvaus) && !empty($alkupaiva) && !empty($loppupaiva) && !empty($opettaja) && !empty($tila) && !empty($aine)) {
        try {
            $sql_lause = "INSERT INTO kurssit 
                (nimi, kuvaus, alkupäivä, loppupäivä, opettaja, tila, aine)
                VALUES (:nimi, :kuvaus, :alkupaiva, :loppupaiva, :opettaja, :tila, :aine)";
            ...
            echo "<p class='warning'>Virhe lisättäessä kurssia: " . htmlspecialchars($e->getMessage()) . "</p>";
        }
    } else {
...
<form method="post">
    ...
    <a href="lista.php" class="btn">Takaisin kurssilistaan</a>
</form>
